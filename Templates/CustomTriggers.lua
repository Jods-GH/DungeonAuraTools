...
JDT.Templates.CustomTriggers.DisposeSuccessRemove = function(triggerData)
    local summonID,duration,removeID = triggerData.summonID,triggerData.duration,triggerData.removeID
    local trigger = {
        customTrigger = "function(s,event,_,subevent,_,sourceGUID,_,_,_,destGUID,_,_,_,spellID)\n    \n    if subevent == \"SPELL_SUMMON\" and spellID == "..summonID.." and not aura_env.GuidList[destGUID] then\n   aura_env.GuidList[destGUID] = true     if not s[\"\"] then\n            \n            s[\"\"] = {\n                duration = "..duration..",\n                expirationTime = GetTime()+"..duration..",\n                stacks = 1,\n                progressType = \"timed\",\n                autoHide = true,\n                changed = true,\n                show = true,\n            }\n        else\n            s[\"\"].stacks = s[\"\"].stacks+1\n            s[\"\"].changed = true\n            s[\"\"].expirationTime = GetTime()+"..duration.."\n        end\n        \n        return true\n        \n        \n    elseif subevent == \"SPELL_CAST_SUCCESS\" and spellID == "..removeID.." and aura_env.GuidList[sourceGUID] then\n    aura_env.GuidList[sourceGUID] = false    \n        if s[\"\"] then\n            \n            s[\"\"].stacks = s[\"\"].stacks-1\n            if s[\"\"].stacks == 0 then\n                s[\"\"].show = false\n            end\n            s[\"\"].changed = true\n            return true \n        end\n        \n    end\nend",
